...
const SQL_RULES = [
  ...
  // ── New rules (v2) ─────────────────────────────────────────────────────
  { name: 'sql-case-when',          severity: 'high',     pattern: /CASE\s+WHEN\s+.*\s+THEN/i,                              description: 'CASE WHEN conditional injection' },
  { name: 'sql-extractvalue',       severity: 'critical', pattern: /EXTRACTVALUE\s*\(/i,                                    description: 'EXTRACTVALUE error-based injection' },
  { name: 'sql-updatexml',          severity: 'critical', pattern: /UPDATEXML\s*\(/i,                                       description: 'UPDATEXML error-based injection' },
  { name: 'sql-json-extract',       severity: 'medium',   pattern: /JSON_EXTRACT\s*\(/i,                                    description: 'JSON_EXTRACT injection' },
  { name: 'sql-sys-schema',         severity: 'critical', pattern: /sys\.(user_summary|processlist|statements_with_errors)/i, description: 'MySQL sys schema access' },
  { name: 'sql-gtid',               severity: 'critical', pattern: /GTID_SUBSET\s*\(/i,                                    description: 'GTID_SUBSET error-based injection' },
  { name: 'sql-exp-tilde',          severity: 'critical', pattern: /exp\(~\(/i,                                             description: 'EXP(~()) error-based injection' },
  { name: 'sql-polygon',            severity: 'high',     pattern: /(polygon|geometrycollection|linestring|multipoint)\s*\(/i, description: 'Geometry function error-based injection' },
  { name: 'sql-procedure-analyse',  severity: 'high',     pattern: /procedure\s+analyse\s*\(/i,                             description: 'PROCEDURE ANALYSE injection' },
  { name: 'sql-having',             severity: 'high',     pattern: /\bHAVING\s+\d+\s*=\s*\d+/i,                            description: 'HAVING tautology' },
  { name: 'sql-rowterminator',      severity: 'high',     pattern: /ROWTERMINATOR/i,                                        description: 'MSSQL ROWTERMINATOR injection' },
  { name: 'sql-dbms-version',       severity: 'critical', pattern: /@@version|@@global|@@session/i,                         description: 'DBMS variable access' },
];
